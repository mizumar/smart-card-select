...
| `betterCardId`  | `parseRate(maxReturnRate)` の数値を比較し、優位なカードの `id` を取得 | 「おすすめ！」バッジおよび勝者チェックアイコンの表示判定 |
...
- **券面・バナー画像表示仕様（実画像対応）**
  - `card.imageUrl` が存在する場合：
    - 従来の枠線（`border`）・背景色・固定アスペクト比（`aspect-[1.58/1]`）を完全撤去。
    - 横幅を `w-36` (`sm:w-40` / 約144px) まで大型化して表示（`shrink-0` で固定）。
    - ASP等から提供されるバナー画像・枠付き画像本来の縦横比を活かし、バナー内の文字やキャンペーン表記の視認性を最大化（`h-auto` / `object-contain`）。
  - `card.imageUrl` が未設定の場合：
    - 従来のグラデーションブランドカラー（`brandColor`）によるコンパクトなCSSモック（`w-24 h-15`）へ自動フォールバック。
...
- **おすすめカード（勝者）の自動ハイライト**
- 選択された2枚のカードの `maxReturnRate` 等の数値を比較し、高スペックな方のカード上部に「おすすめ！」バッジ（Trophyアイコン）を表示。

- **中央セパレーター付きグリッドテーブルUI**
- `年会費` / `最大還元率` / `基本還元率` を並列表示する2列レイアウト。

- 高還元率側の項目に `CheckCircle2` アイコンを付与。

- **同時遷移（Wコンバージョン）アクション**
- 個別カードの `affiliateUrl` ボタンに加え、モーダル下部に「両方の公式サイトを同時に開く」ボタン（`window.open` で2タブ同時起動）を配置。
...
### 5.8 モーダル内カード券面表示仕様（実画像対応）(CompareBottomSheet.tsx)

- **条件分岐表示:**
  - `card.imageUrl` が存在する場合: ASP提供画像（バナー画像）を表示。
  - `card.imageUrl` が未設定の場合: 従来のグラデーションブランドカラー（`brandColor`）によるCSSモック券面を表示。
- **レイアウト・サイズ統一（アスペクト比固定）:**
  - 画像有無にかかわらず `aspect-[1.58/1]` を維持し、実画像とCSSモックで券面表示エリアの高さを揃えてモーダル内のレイアウト崩れ・ガタつきを防止。
  - 横幅は `w-36` (`sm:w-40`) の大型表示とし、バナー画像内の文字・ロゴの視認性を確保。
  - 余計な枠線（`border`）や枠内パディングを廃止し、`object-contain` により画像本来の縦横比で拡大表示。
- **縦スペース調整（フレックススペーサー）:**
  - 券面画像下部に最低限の固定空白（`h-2`）と可変スペーサー（`flex-1`）を配置し、カード名やスペック行の高さが左右で異なる場合でも比較表の開始位置を均一に揃える構造を適用。
